...
export default async function ServiceDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const biology = getPestBiology(service.slug);
  ...
  return (
    <>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <article className="rounded-3xl bg-forest-deep p-8 text-cream shadow-[var(--shadow-lifted)]">
            ...
            <div className="mt-6 rounded-2xl bg-cream/10 p-4 text-sm text-cream/85 ring-1 ring-inset ring-cream/15">
              Untreated {service.short.toLowerCase()} problems compound month after month. A single
              inspection tells you exactly what you&apos;re dealing with.
            </div>
          </article>
        </div>
      </Section>
      ...
      >
        <ol className="space-y-4">
          {content.process.map((p) => (
            <li
              key={p.step}
              className="flex items-start gap-5 rounded-3xl border border-border bg-card p-6 shadow-[var(--shadow-card)]"
            >
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-forest-deep font-serif text-base text-amber">
                {p.step}
              </span>
      ...
      {/* Biology & Integrated Management deep-dive */}
      {biology && <PestBiologySection data={biology} />}

      {/* Why AIPS */}
      <Section
        bg="cream"
        eyebrow="Why AIPS"
        title="Built by agricultural scientists, not call-centre sales."
      >
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            {
              t: "15+ years in the field",
              b: "Founded 2010 · 40,000+ homes & businesses protected.",
            },
            {
              t: "CIB-approved chemistry",
              b: "Envu, FMC, Syngenta, UPL — strict label-rate dilutions only.",
            },
            {
              t: "Same technician, every visit",
              b: "Trained in-house for 90+ hrs. No sub-contractors.",
            },
            {
              t: "Transparent reporting",
              b: "Every visit ends with a written job report — what was applied, where and why.",
            },
          ].map((c) => (
          ...
          <div className="grid gap-8 md:grid-cols-[1.4fr_auto] md:items-center">
            <div>
              ...
              <p className="mt-2 text-cream/80">
                A trained AIPS technician will visit, inspect, quote — and only then treat. No
                pressure, no upsell.
              </p>
            </div>
