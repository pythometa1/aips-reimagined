...
export function InspectionForm({ variant = "card" }: { variant?: "card" | "flat" }) {
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    console.info("[AIPS inspection request]", data);
    setSent(true);
  };

  const wrap =
    variant === "card"
      ? "rounded-3xl bg-card p-6 shadow-[var(--shadow-lifted)] sm:p-8"
      : "";
      ...
      {sent ? (
        <div className="flex items-start gap-3 rounded-2xl bg-forest-deep/5 p-5 text-sm text-forest-deep">
          ...
          <div>
            ...
            <p className="text-forest-deep/80">
              A trained technician will call you back within business hours. For anything urgent, call
              +91 94774 01805 or WhatsApp us.
            </p>
          </div>
        </div>
      ) : (
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input id="phone" name="phone" placeholder="9XXXXXXXXX" type="tel" required className="mt-1" />
          </div>
          <div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              >
                ...
                {services.map((s) => (
                  <option key={s.slug} value={s.slug}>{s.short}</option>
                ))}
